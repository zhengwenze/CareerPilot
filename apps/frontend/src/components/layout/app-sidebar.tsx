    ...
    <>
      ...
      {!collapsed && (
        <span className="min-w-0 flex-1">
          <span className="block truncate font-mono text-sm font-bold uppercase">{item.title}</span>
          {item.description ? (
            <span className="mt-0.5 block truncate font-mono text-xs">{item.description}</span>
          ) : null}
        </span>
      )}
        ...
        <div className="flex items-center gap-3">
          ...
          {!collapsed ? (
            <div className="min-w-0 flex-1">
              ...
              <p className="truncate font-mono text-xs text-black">AI 求职工作台</p>
            </div>
          ) : null}
          <Button
  ...
  return (
    <>
      ...
      <div className="fixed bottom-4 left-4 z-50 md:hidden">
        <Button onClick={() => setMobileOpen(true)} size="icon-lg" type="button" variant="primary">
